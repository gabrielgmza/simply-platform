...
export default function ConfirmarPage() {
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      // Construir el endpoint de destino para el motor
      ...
      const res = await fetch("/api/transfer-engine/execute", {
        method: "POST",
      ...
      if (!res.ok) throw new Error(data.message || "Error procesando");
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-center text-xs text-white/40">
        En el próximo paso te diremos cómo enviar el dinero a Simply
      </p>
    </div>
  );
}
